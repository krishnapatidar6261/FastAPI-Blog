...
class HashTags(Base):
    ...
    blogs = relationship("Blogs", secondary=blog_hashtag, back_populates="hashtags")
...
class Blogs(Base):
    ...
    hashtags = relationship("HashTags", secondary=blog_hashtag, back_populates="blogs")
    ...
    def serializer(self, user_id=None):
        ...
        if user_id:
            dic["is_liked"] = (
                self.likes.filter_by(liked_by_user_id=user_id).first() is not None
            )
