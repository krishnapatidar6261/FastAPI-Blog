...
class BlogModelCRUDServices:
    ...
    def add_hashtags_blog(self, blog_obj, hashtag_ids: list[int]):
        ...
        hashtags_queryset = hashtag_service.get_hashtag_queryset_by_ids(
            ids_list=hashtag_ids
        )
    ...
    def create_blog(self, schema: BlogCreateSchema, user_id: int):
        blog_obj = Blogs(title=schema.title, content=schema.content, user_id=user_id)

        if schema.hashtag_ids:
            self.add_hashtags_blog(hashtag_ids=schema.hashtag_ids, blog_obj=blog_obj)
    ...
    def update_blog(self, user_id, schema: BlogUpdateSchema):
        ...
        if schema.hashtag_ids:
            self.add_hashtags_blog(hashtag_ids=schema.hashtag_ids, blog_obj=blog)
        try:
            self.save()
        except Exception as e:
            return api_response(500, message=f"Internal Server error: {str(e)}")
...
class BlogHashTagsService(DBPingService):
    def get_all_hashtags(self, offset, limit):
        ...
        return api_response(
            status_code=200, message="Fetched Successfully", data=paginated_res
        )
