...
class UserRegistrationLoginService:
    ...
    def register_user(cls, db: Session, schema):
        ...
        return api_response(
            200, data=new_user.serializer, message="Register successfully"
        )
    ...
    def login_user(cls, db: Session, schema):
        ...
        password_verified = PassswordUtils.verify_password(
            plain_password=schema.password, hashed_password=user_obj.password
        )
        if password_verified:
            ...
            return api_response(
                200, message="Username and Password verified", data=tokens_dic
            )
